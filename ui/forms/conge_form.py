...
from utils.date_utils import validate_date, format_date_for_display, get_holidays_set_for_period, calculate_reprise_date
...
class CongeForm(tk.Toplevel):
    ...
    def _update_reprise_date(self):
        """Met à jour le champ de la date de reprise à partir de la date de fin."""
        self.reprise_date_entry.config(state="normal")
        self.reprise_date_entry.delete(0, tk.END)
        
        end_date = validate_date(self.end_date_entry.get())
        if end_date:
            holidays_set = get_holidays_set_for_period(self.db, end_date.year, end_date.year + 1)
